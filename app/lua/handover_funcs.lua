...
function HANDOVER_check_status(port)
    local wb = yarp.Bottle()
    local reply = yarp.Bottle()
    wb:clear()
    wb:addString("check_status")
    port:write(wb,reply)
    return reply:get(0):asString()
end
...
function HANDOVER_reach_pose(port)
   local wb = yarp.Bottle()
   local reply = yarp.Bottle()
   wb:clear()
    wb:addString("reach_pose")
    wb:addInt(0)
    port:write(wb,reply)
    --yarp.Time_delay(5.0)
    return reply:get(0):asString()
...
function HANDOVER_go_back_home(port)
   local wb = yarp.Bottle()
   local reply = yarp.Bottle()
   wb:clear()
