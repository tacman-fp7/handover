...
*************************************************************************/
    ...
    void iKinInvertedLink::setAlpha(const double _Alpha)
    {
        Alpha=_Alpha;
        c_alpha=cos(Alpha);
        s_alpha=sin(Alpha);
...
*************************************************************************/
    ...
    // Matrix iKinChainMod::aJacobian()
    // {
        
    // }

    // Matrix iKinChainMod::dJacobian()
    // {
        
    // }

    // Matrix iKinChainMod::alphaJacobian()
    // {
    ...
    bool iCubCustomLimb::alignJointsBounds(const deque<IControlLimits*> &lim)
    {
        ...
        else if (getType() == "RtoL" || getType() == "RHtoL")
        {
            ...
            {   
                ...
                // printf("old lims %g %g \t", (*this)[iLeft+1+iRight].getMin()*CTRL_RAD2DEG, (*this)[iLeft+1+iRight].getMax()*CTRL_RAD2DEG);
                (*this)[iRight+1+iLeft].setMin(CTRL_DEG2RAD*min);
...
*************************************************************************/
    ...
    /************************************************************************/
    iCubShoulderConstrMod::iCubShoulderConstrMod(iKinChainMod *_chain, char _readingmode, int _sh)
                          : iKinLinIneqConstr(), chain(_chain), readingmode(_readingmode), sh(_sh)
    {
    ...
    void iCubShoulderConstrMod::update(void*)
    {
        ...
        {   
                ...
                !(*chain)[sh+2].isBlocked())
            {
                ...
                // // constraints to prevent arm from touching torso
                row.zero();
                row[offs+1]=-1.0; row[offs]=shou_m;
                appendMatrixRow(_C,row);
            ...
            // if elbow and pronosupination axes are controlled, constraint them
            if (!(*chain)[sh-1].isBlocked() && !(*chain)[sh-2].isBlocked())
            {
                // compute offset to elbow's axis
                // given the blocked/release status of
                // previous link
